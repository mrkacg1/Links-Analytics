...
class Tools {
    ...
    private function tool_deleteurls($request){
        ...
        $urls = DB::url()->where('click', 0)->whereRaw('date < (CURDATE() - INTERVAL 30 DAY)')->findMany();
    ...
    private function tool_cleanup(Request $request){
        ...
        $usercount = 0;
        if($request->users){            
            foreach(DB::user()->select('id')->whereNull('teamid')->whereRaw('DATE(date) < (CURDATE() - INTERVAL 30 DAY)')->find() as $user){
