...
class Subscription {
    ...
    */
    public function checkout(Request $request, int $id, string $type){        
        ...
        if(\Helpers\App::isExtended() && $subscription = DB::subscription()->where('userid', $user->id)->where('status', 'Active')->first()){
        ...
        if($user->planid == $id && !$user->trial) return Helper::redirect()->to(route('pricing'))->with('danger', e('You already subscribed to this plan. If you want to upgrade, please choose another plan.'));
