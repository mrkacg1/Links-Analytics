...
class Gate {
    ...
    public static function profile($profile, $user = null, $url = null){
        ...
        if(isset($profiledata['settings']['share']) && $profiledata['settings']['share']){
            View::push("<script>
                    if(navigator.share){
                        $('body').prepend('<a href=\"#\" data-trigger=\"share\" class=\"btn btn-white btn-icon-only shadow rounded-circle position-absolute zindex-101 right-4 top-4 d-flex\" data-toggle=\"tooltip\" data-placement=\"right\" title=\"".e('Share')."\"><span class=\"btn-inner--icon\"><i data-feather=\"share\" class=\"text-dark\"></i></span></a>');
                    }
        ...
        if(isset($profiledata['style']['font']) && !empty($profiledata['style']['font'])){
            if(!in_array($profiledata['style']['font'], ['Arial', 'Helvetica Neue', 'Courier New', 'Times New Roman', 'Comic Sans MS', 'Verdana', 'Impact'])){
                View::push('https://fonts.googleapis.com/css?family='.$profiledata['style']['font'])->toHeader();
            }
