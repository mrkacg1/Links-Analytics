...
class Paypal{
    ...
    public static function webhook(Request $request){
        ...
        if($verified){
            ...
            if(\Helpers\App::isExtended() && !$request->custom && $request->product_name){
                ...
                if($subscription->plan == "yearly"){
                    ...
                    $new_expiry = date("Y-m-d H:i:s", strtotime("+1 year", strtotime($subscription->date)));

                }else{

                    $new_expiry = date("Y-m-d H:i:s", strtotime("+1 month", strtotime($subscription->date)));
                }
